...
class BupDiarySupabaseUser extends BaseAuthUser {
  BupDiarySupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> bupDiarySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BupDiarySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
